...
def extrae_np(archivo, n=-1, rescale=1.0):
    """
    ...
    # Calcular nuevas dimensiones si se aplica reescalado
    nuevo_ancho = int(ancho * rescale)
    nuevo_alto = int(alto * rescale)

    arreglo_datos = np.zeros((num_cuadros, nuevo_alto, nuevo_ancho, 6))
    i = -1
    while cap.isOpened():
        ...
        # Reescalar el cuadro
        cuadro_bgr = cv.resize(cuadro_bgr, (nuevo_ancho, nuevo_alto))
...
def obtén_tabla_datos(nombre="sponge1", conjuntos=[0, 30, 75], rescale=1.0):
    """
    ...
    arreglo_datos_4D = extrae_np(archivos_fuente["sponge1"], -1, rescale=rescale)
